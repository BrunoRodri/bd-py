...
def update_pedido(id_pedido):
    try:
        ...
        if resultado_busca:
            ...
            if resultado_atualizacao.modified_count == 1:
                return jsonify({"message": f"Pedido {id_pedido} atualizado com sucesso."}), 200
            else:
                return jsonify({"error": "Erro ao atualizar pedido."}), 500
        else:
            return jsonify({"error": f"Pedido com id {id_pedido} não encontrado."}), 404
...
def delete_pedido(id_pedido):
    try:
        ...
        if resultado_busca:
            ...
            if resultado.deleted_count == 1:
                return jsonify({"message": f"Pedido {id_pedido} excluído com sucesso."}), 200
            else:
                return jsonify({"error": f"Erro ao excluir o pedido."}), 500
        else:
            return jsonify({"error": f"Pedido com id {id_pedido} não encontrado."}), 404
